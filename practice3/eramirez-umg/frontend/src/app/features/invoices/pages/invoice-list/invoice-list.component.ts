...
<div class="table-scroll"><table class="grid-table"><thead><tr><th>Number</th><th>Supplier</th><th>Issue date</th><th>Total</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr *ngFor="let invoice of invoices"><td>{{ invoice.number }}</td><td>{{ invoice.supplierName }}</td><td>{{ invoice.issueDate | date:'mediumDate' }}</td><td>&dollar;{{ invoice.total | number:'1.2-2' }}</td><td>{{ invoice.status }}</td><td><a [routerLink]="['/invoices', invoice.id]">View details</a> <a *ngIf="auth.canManageInvoices()" [routerLink]="['/invoices/edit', invoice.id]">Edit</a> <button *ngIf="auth.canDeleteInvoices()" class="text-danger" (click)="deleteInvoice(invoice.id)">Delete</button></td></tr></tbody></table></div>
