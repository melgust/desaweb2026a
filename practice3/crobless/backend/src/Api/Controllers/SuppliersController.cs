...
public class SuppliersController(IConfiguration configuration) : ControllerBase
{
    [Route("")]
    [Route("{**path}")]
    ...
    public IActionResult Deprecated(string? path = null)
    {
        ...
        Response.Headers["Link"] = $"<{url}/suppliers>; rel=\"successor-version\"";
        return StatusCode(410, new { message = "Este módulo se migró a Spring Boot.", serviceUrl = $"{url}/suppliers" });
    }
}
