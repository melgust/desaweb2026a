...
public class ProductService : IProductService
{
    ...
    {
        var p = await _db.Products.Include(p => p.Category).FirstOrDefaultAsync(p => p.Id == id, ct)
            ?? throw new KeyNotFoundException("Product not found.");
        return new ProductDto(p.Id, p.Name, p.Description, p.Price, p.Stock, p.IsActive, p.CreatedAt, p.CategoryId, p.Category?.Name);
    }

    public async Task<ProductDto> CreateAsync(CreateProductRequest request, CancellationToken ct)
    {
        if (request.CategoryId.HasValue && !await _db.Categories.AnyAsync(c => c.Id == request.CategoryId, ct))
        ...
        var categoryName = request.CategoryId.HasValue ? await _db.Categories.Where(c => c.Id == request.CategoryId).Select(c => c.Name).FirstOrDefaultAsync(ct) : null;
        return new ProductDto(p.Id, p.Name, p.Description, p.Price, p.Stock, p.IsActive, p.CreatedAt, p.CategoryId, categoryName);
    }
